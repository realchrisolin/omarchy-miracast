...
| `audioEnabled` | `true` | Creates a PipeWire/Pulse **Miracast** null sink for capture. The **default output switches to Miracast only after PLAY/streaming** (WirePlumber often prefers a new sink during handshake — we hold the previous default until then). FluxCast captures **`miracast.monitor`**. App streams follow the default (route watcher). LPCM-only TVs use WFD `stream_type=0x83`; keep the Miracast sink near **100%** or the TV sounds faded. See [BUILD.md §6](BUILD.md). |
