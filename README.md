...
| `audioEnabled` | `true` | Creates a PipeWire/Pulse **Miracast** null sink (selectable in Sound). FluxCast captures **`miracast.monitor`** via `pw-cat --target` (not the mic). App streams follow the Sound-panel default (route watcher). LPCM-only TVs use WFD `stream_type=0x83`; keep the Miracast sink near **100%** or the TV sounds faded. See [BUILD.md §6](BUILD.md). `--no-audio` / `audioEnabled: false` disables. |
