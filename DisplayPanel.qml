        ...
        Column {
          ...
          Row {
            ...
            component LinkMetricCol: Column {
              ...
              // Long encode strings need wrap; radio metrics stay single-line elide.
            ...
            Column {
              ...
              // Next free slot after Freq. removal — capture method + encoder.
              LinkMetricCol {
                label: "Codec"
                wrapValue: true
                value: {
                  var enc = String(miracast.encoder || "").trim().toLowerCase()
                  var path = String(miracast.capturePath || miracast.captureEncode || "").toLowerCase()
                  var lines = []
                  // Method = how frames reach the encoder (DMA-BUF vs pipe vs CPU).
                  if (path === "dmabuf")
                    lines.push("Method: GPU + DMA-BUF")
                  else if (path === "pipe" || path === "vaapi")
                    lines.push("Method: GPU")
                  else if (path === "cpu")
                    lines.push("Method: CPU")
                  else if (path !== "")
                    lines.push("Method: " + path)
                  else
                    lines.push("Method: --")
                  // Encoder = which codec implementation compresses the video.
                  if (enc === "h264_vaapi")
                    lines.push("Encoder: H.264 VAAPI")
                  else if (enc === "h264_nvenc" || enc === "nvenc")
                    lines.push("Encoder: H.264 NVENC")
                  else if (enc === "h264_amf" || enc === "amf")
                    lines.push("Encoder: H.264 AMF")
                  else if (enc === "h264_qsv" || enc === "qsv")
                    lines.push("Encoder: H.264 QSV")
                  else if (enc === "libx264" || enc === "x264")
                    lines.push("Encoder: H.264 x264")
                  else if (enc.indexOf("hevc") >= 0 || enc.indexOf("h265") >= 0)
                    lines.push("Encoder: H.265 HEVC")
                  else if (enc !== "")
                    lines.push("Encoder: " + enc)
                  else
                    lines.push("Encoder: --")
                  return lines.join("\n")
                }
            ...
            Column {
              ...
              // Next free slot — RC + QP/quality knobs.
              LinkMetricCol {
                label: "Encode"
                wrapValue: true
                value: {
                  var rc = String(miracast.vaapiRcMode || "").trim().toUpperCase()
                  var lines = []
                  if (rc === "CQP")
                    lines.push("Mode: CQP")
                  else if (rc === "CBR")
                    ...
                    lines.push("Mode: QVBR")
                  else if (rc !== "")
                    lines.push("Mode: " + rc)
                  else
                    lines.push("Mode: --")
                  if (miracast.vaapiQp !== null && miracast.vaapiQp !== undefined
                      && String(miracast.vaapiQp) !== "")
                    lines.push("QP: " + String(miracast.vaapiQp))
                  if (String(miracast.vaapiQuality || "") !== "")
                    lines.push("Quality: " + String(miracast.vaapiQuality))
                  if (String(miracast.vaapiIQfactor || "") !== "" && rc === "CQP")
                    lines.push("I-frame: ×" + String(miracast.vaapiIQfactor))
                  return lines.join("\n")
                }
              }
            }
