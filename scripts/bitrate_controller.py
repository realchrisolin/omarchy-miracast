...
# Defaults tuned for Miracast 1080p (Smart View used 1920×1080@60 on 5 GHz).
# Samsung strings: updateConfiguration min/max/init + minQP/maxQP.
DEFAULT_INIT_KBPS = 10_000
DEFAULT_MIN_KBPS = 1_500
DEFAULT_MAX_KBPS = 18_000
DEFAULT_QP_MIN = 18
DEFAULT_QP_MAX = 42
...
def config_for_band(
    *,
    ...
    if band_5ghz:
        # Smart View live session was 5 GHz / 1080p60 — allow higher ceiling.
        return BitrateConfig(
            init_kbps=10_000,
            min_kbps=2_000,
            max_kbps=20_000 if (width_mhz or 20) >= 40 else 15_000,
            qp_min=16,
            qp_max=40,
            rc_mode="QVBR",
        )
    # 2.4 GHz / 20 MHz MCC — conservative max (corruption line ~20 Mbps class).
    return BitrateConfig(
        init_kbps=12_000,
        min_kbps=1_500,
        max_kbps=15_000,
...
def _clamp_kbps(kbps: int, cfg: BitrateConfig, capacity_mbps: Optional[float]) -> int:
    ...
    if capacity_mbps is not None and capacity_mbps > 1.0:
        # Leave headroom on the air (Samsung also respects network status).
        hi = min(hi, int(capacity_mbps * 1000 * 0.45))
        hi = max(hi, lo)
