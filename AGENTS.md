...
- **Quiet P2P by default = SCC.** Opt-in quiet CSA only (`p2pQuietCsa`). For air/UI freq: prefer peer **`oper_freq` when 5 GHz**; only fall back to `listen_freq` when iw GO merely mirrors STA 5 GHz and the peer has no 5 GHz oper (Intel MCC “iw lie”). Dual idle NIC preferred over sharing STA (`p2pWifiInterface=auto`). USB GO drops 5 GHz by default (`p2pGo5GHz=false`); dedicated USB `wpa_supplicant` is `usbWpaSupplicant` / `FLUXCAST_USB_WPA` (never a hardcoded path in FluxCast).
...
- **Encoding strategy** (`encodeStrategy`): **`smartview`** (default) = QVBR + BitrateController on **DMA-BUF** (Samsung dig: **min 2M / init 8M / max 14M**, QP bounds **15–44**). Use `prepare_dmabuf_settings()` on start/restart; pref=`dmabuf` stays sticky across SIGUSR1. **VAAPI QVBR target hit:** bitrate-driven (`qmin`/`qmax`, not fixed `qp=`), `minrate`≈70% of target, VBV ~2s — needs the **custom wf-recorder** build that applies `rc_min_rate` on `AVCodecContext` (set `wfRecorderBin`; stock PATH binary undershoots to ~3 Mbps). Starve→pipe only if air TX stays **&lt;~2.5 Mbps** absolute. **`performance`** = DMA-BUF **CQP** (Best uses QP ladder — no `set-bitrate-kbps`). Pipe: avoid **quality &lt; 3** / **async &gt; 1**.
- **Best (Dynamic) + QVBR ABR:** `bitrate_controller.py` decide() every tick; **coalesce** SIGUSR1 applies (Δqp≥2 or Δkbps≥15%, min ~45s). **Do not** invent LOSS from iw `retry%` (Smart View uses RTCP RR). Demote only on NetStall/fps sag or hard `tx_failed`. Under **performance** → DMA-BUF **QP 1–51** ladder (`dynamic_encode.py`). **`encodeBestLocked`** freezes the step.
