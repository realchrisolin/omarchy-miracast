...
- **Encoding strategy** (`encodeStrategy`): **`smartview`** (default) = QVBR + BitrateController on **DMA-BUF** (`FLUXCAST_WFD_DMABUF_STICKY`); wf-recorder must restore fps after filters. Fallback to pipe only if air TX stays **&lt;~2.5 Mbps** absolute (quiet QVBR at qp22 is often 4–8 Mbps — not starve). **`performance`** = DMA-BUF **CQP** only (Best uses QP ladder — no `set-bitrate-kbps`). Pipe: avoid **quality &lt; 3** / **async &gt; 1** (quality=1 hung → `bufs_size` / audio-only).
