...
- **Encoding strategy** (`encodeStrategy`): **`smartview`** (default) = QVBR + BitrateController on **DMA-BUF** (Samsung dig: **min 2M / init 8M / max 14M**, QP **15–44**). Use `prepare_dmabuf_settings()` on start/restart; pref=`dmabuf` stays sticky across SIGUSR1. Starve→pipe only if air TX stays **&lt;~2.5 Mbps** absolute (quiet QVBR ~4–8 Mbps is fine). **`performance`** = DMA-BUF **CQP** (Best uses QP ladder — no `set-bitrate-kbps`). Pipe: avoid **quality &lt; 3** / **async &gt; 1**.
- **Best (Dynamic) + QVBR ABR:** `bitrate_controller.py` decide() every tick; **coalesce** SIGUSR1 applies (Δqp≥2 or Δkbps≥15%, min ~45s). **Do not** treat sticky iw `retry%` `loss:` as urgent (that caused ~17s pause storms). QP-only demotes at the bitrate floor never restart. Under **performance** → DMA-BUF **QP 1–51** ladder (`dynamic_encode.py`). **`encodeBestLocked`** freezes the step.
